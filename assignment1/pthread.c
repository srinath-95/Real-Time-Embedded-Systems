...
// This code was written by Professor Sam Siewert. I am using this code for educational purspose to understand the concept of pthreads and scheduling them.
// I have by no means have any intention to duplicate the work
// All copyrights reserved to the owner of the code
 ...
 */
 /* Calculates the fibonaci series for 10 ms period*/
void *fib10(void *threadp)
{
   ...
   while(!abortTest)
   {
       ...
       cpucore=sched_getcpu();	//determines on which CPU the thread is running
       printf("F10 start %d @ %lf on core %d\n", release, (event_time=getTimeMsec() - start_time), cpucore);

       do
       {
           FIB_TEST(seqIterations, FIB_TEST_CYCLES);	//calls the fibonaci calculation function
           limit++;
       }
 ...
 /* Calculates the fibonaci series for 20 ms period*/
void *fib20(void *threadp)
{
   ...
   while(!abortTest)
   {
        sem_wait(&semF20);	//decrements the semaphore F20
...
/* Prints the Pthread scheduler policy*/
...
/* Sequencer - acts as a scheduler for fib10 and fib20*/
...
void main(void)
{
   ...
   /*Clearing all the CPU sets*/
   CPU_ZERO(&allcpuset);

   for(i=0; i < NUM_CPU_CORES; i++)
       CPU_SET(i, &allcpuset);	//Adding the CPU to set

   printf("Using CPUS=%d from total available.\n", CPU_COUNT(&allcpuset));	//Gives the total number of CPUs that is set


    // initialize the sequencer semaphores 
    /* setting the semaphore to share between the threads in the process, this would located at an address that is global to all threads */
	...
	/* Gets the priority value that can be used with scheduling algorithm*/
    rt_max_prio = sched_get_priority_max(SCHED_FIFO);	// getting the maximum priority value
    rt_min_prio = sched_get_priority_min(SCHED_FIFO);	// getting the minimum priority value

    rc=sched_getparam(mainpid, &main_param);
	//Setting the main process with highest priority
    main_param.sched_priority=rt_max_prio;
    rc=sched_setscheduler(getpid(), SCHED_FIFO, &main_param);	//sets the scheduling policy to SCHED_FIFO
    if(rc < 0) perror("main_param");
    print_scheduler();	//prints the scheduling policy

	/*Gets the scope of the pthread*/
    ...
    for(i=0; i < NUM_THREADS; i++)
    {
      ...
      rc=pthread_attr_init(&rt_sched_attr[i]);	//initializes the thread attributes 
      rc=pthread_attr_setinheritsched(&rt_sched_attr[i], PTHREAD_EXPLICIT_SCHED);
      rc=pthread_attr_setschedpolicy(&rt_sched_attr[i], SCHED_FIFO);
      rc=pthread_attr_setaffinity_np(&rt_sched_attr[i], sizeof(cpu_set_t), &threadcpu);

      rt_param[i].sched_priority=rt_max_prio-i;	//Setting the priority to each threads using FIFO
      pthread_attr_setschedparam(&rt_sched_attr[i], &rt_param[i]);	//sets the scheduling paramters attributes of the thread attributes
   ...
   for(i=0;i<NUM_THREADS;i++)
       pthread_join(threads[i], NULL);	//waits for the threads to complete
